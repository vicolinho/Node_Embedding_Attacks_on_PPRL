...
def bipartite_graph_edges_to_matches(G_edges, nodes1, nodes2, no_top_pairs, mode): #r
    if mode == 'shm':
...
def bipartite_graph_edges_to_matches_mwm(G_edges, nodes1, nodes2, no_top_pairs): #r
...
def embeddings_to_binary_lsh(embeddings1, embeddings2, dim): #r
...
def create_dicts_lsh_embeddings(binary_lsh1, binary_lsh2, ids_list_hamming_lsh): #r
...
def add_id_to_lsh_dict(i, j, ids_hamming_lsh, lsh_dict, lsh_emb): #r
...
def embeddings_to_bipartite_graph_edges_lsh(embeddings1, embeddings2, settings, weights): #r
...
def embeddings_to_bipartite_graph_edges_lsh_dicts(embeddings1, embeddings2, lsh_dicts_embeddings, threshold, vidanage_weights, weights): #r
...
def weighted_cos_sim(cos_sims_list, weights): #r
    try:
...
def bipartite_graph_edges_to_matches_shm(edges, nodes1, nodes2, no_top_pairs): #r
    if len(edges) == 0:
        return []
    try:
...
def bipartite_graph_edges_to_matches_smm(edges, nodes1, nodes2, no_top_pairs): #r
...
def transform_edges_df_to_vidanage(df, w_cos, w_degr_conf, w_sim_conf): #r
...
def matches_from_embeddings_two_graphs(embeddings1, embeddings2, nodes1, nodes2, settings, weights, prefix_char=False): #r
...
def matches_from_embeddings_combined_graph(embedding_results, id1, id2, settings, weights = [1.0]): #r
...
def split_embeddings_by_nodes(embeddings, nodes, prefix1, prefix2): #r
    embeddings1 = []
    embeddings2 = []
...
def remove_prefix_from_matches(matches): #r
    adapted_matches = []
...
def matches_from_priority_queue(highest_pairs): #r
    matches = []
