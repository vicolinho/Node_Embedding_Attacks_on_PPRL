...
def import_data_plain(filename, max_record_count, qgram_attributes, blk_attributes, encoded_attr):
    plain_data = pd.read_csv(filename, na_filter=False)
    if max_record_count > 0:
        plain_data = plain_data.head(max_record_count)
    return preprocessing.preprocess_plain_df(plain_data, qgram_attributes, blk_attributes, encoded_attr)
